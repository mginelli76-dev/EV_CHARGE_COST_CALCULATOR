...
# 2. Selezione Profilo
profilo = st.selectbox("Tipo di ricarica", 
                      ["Colonnina (0.81€ - 92% eff.)", "Casa (0.25€ - 85% eff.)"])

# Imposta valori in base al profilo
if "Colonnina" in profilo:
    prezzo_default = 0.81
    eff = 0.92
else:
    prezzo_default = 0.25
...
# 3. Altri Input
prezzo = st.number_input("Prezzo reale (€/kWh)", value=prezzo_default, format="%.2f")

col1, col2 = st.columns(2)
with col1:
    inizio = st.number_input("Batteria Inizio (%)", value=44, min_value=0, max_value=100)
with col2:
    fine = st.number_input("Batteria Fine (%)", value=80, min_value=0, max_value=100)
